    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="!max-w-4xl max-h-[90vh] overflow-y-scroll">
        ...
        <div className="mt-4">
          ...
          <p>
            <strong>Invoice Number:</strong> {order?.razorpayInvoice?.number}
          </p>
          {order?.razorpayInvoice && (
            <Button className="bg-transparent border text-darkColor/80 mt-2 hover:text-darkColor hover:border-darkColor hover:bg-darkColor/10 hoverEffect ">
              {order?.razorpayInvoice?.invoice_url && (
                <Link href={order?.razorpayInvoice?.invoice_url} target="_blank">
                  Download Invoice
                </Link>
              )}
            </Button>
